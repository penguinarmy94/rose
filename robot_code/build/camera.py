...
class Camera():
    ...
    def __init__(self, queue = None, pin = 13, pos = 7, capture_path = "/tmp/"):
        if queue and pin:
            ...
            if not self.__queue.empty():
                ...
                try:
                    if self.__interval > 0:
                        ...
                        seconds_passed = (now - self.__last_capture).total_seconds()
                        if seconds_passed >= self.__interval:
    ...
    def read_queue(self):
        ...
        if message_packet["type"] == "position":
            ...
            pos = self.__pos + 3.1 * float(message_packet["message"])
